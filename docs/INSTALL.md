...
一般的なレンタルサーバーではアップロードするだけで動作します。
権限変更が必要な場合も、まず `0755` を使用し、安易に `0777` にはしないでください。

## 3. ブラウザで開く

アップロード先の `index.php` をブラウザで開きます。
初期状態でSQLiteデータベースは初回アクセス時に`var/data/edanote.db` として自動作成されます。
